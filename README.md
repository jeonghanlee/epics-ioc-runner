...
## Key Highlights
* **No Python or Framework Runtime**: Built on POSIX-standard `bash` and native `systemd`. Runtime depends on `procServ`, `con`, `lsof`, `ss`, and `journalctl`; no Python interpreter or application framework is required.
