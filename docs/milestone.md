...
deliberate exclusion in the Ledger). The remaining 1.2.0 work is **M19** (U003 local-mode log rotation — design
fixed 2026-06-23: user systemd timer + per-user logrotate `weekly`+`maxsize`,
issue prepared `work/issue-u003-local-logrotate.md`, GitHub filing pending) and
**M12** (#68) sudoers wrapper; the release gate renumbered to **M20**. **M13** (#96) and **M14** (#97) closed 2026-06-17 (the no-op
...
| M19 | 1.2.0 | #TBD local-mode procServ log rotation (per-user logrotate via a user systemd timer) | Carry-forward (U003 / UD-M10-A) | Open | **Design fixed 2026-06-23.** Closes the M10 interim unbounded-local-log gap (UD-M10-A) for a crash-looping `--user` IOC. Scheduler = a user systemd timer (`~/.config/systemd/user/`, on-model with the user IOC units, inherits the linger requirement); trigger = `weekly` + `maxsize` + `rotate 8` + `copytruncate` + `compress` + `nodateext`, timer firing daily so `maxsize` catches fast crash-loop growth between weekly marks; per-user `--state` file under `$XDG_STATE_HOME` (the default is root-only); no `su` directive (local dir `0750 user:user`); deployed by `--local install`, removed by `--local uninstall`; LOG_LAYOUT.md section 5 updated. Issue prepared (`work/issue-u003-local-logrotate.md`), GitHub filing pending (owner). P3-low. |
| M19.T1 | 1.2.0 | install deploys timer+service+config; `logrotate -d` validates; forced run rotates via copytruncate without IOC restart; `maxsize` path rotates on the daily timer | Test sub | Open | — |
| M19.T2 | 1.2.0 | both lifecycle suites green; `--local uninstall` removes the timer, service, and config (no orphan) | Test sub | Open | — |
| M19.T3 | 1.2.0 | both goldens (rocky8 239, debian13 257); LOG_LAYOUT.md section 5 documents local rotation | Test sub | Open | — |
