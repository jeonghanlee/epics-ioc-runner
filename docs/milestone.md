...
**Next session entry point:** **M20 release-gate verification PASSED 2026-06-30 (M20.T1/T2/T3 all green, both goldens self-baked on alsucl-psrv3) — the only remaining 1.2.0 step is the release sequence: master merge + `1.2.0` annotated tag + GitHub release (CHANGELOG notes) + milestone close + two-releases-back branch delete.** Historical journey below: **M11 (#67) and M10 (#54) both closed 2026-06-22**
...
| M20 | 1.2.0 | release gate (no GitHub issue; defined by `testplan_1.2.0.md` "Release Gate") | Release gate | Open | **Gate verification PASSED 2026-06-30** (M20.T1/T2/T3 all green, both goldens). Remaining: the release sequence itself -- master merge + `1.2.0` annotated tag + GitHub release (CHANGELOG notes) + milestone close + two-releases-back branch delete. |
