...
coherence finding CI-1; `Closes #75` on master merge). Next entry point: #79
(prerequisite wording, CI-2) and #80 (crash-detection comment, CI-3), both
1.1.1 docs/comment cleanups surfaced by the 2026-06-04 coherence sweep. Version
...
| 1.1.1 | #79 align local-mode tool prerequisite wording with the `~/.local/bin` resolver order | Coherence (CI-2) | Open | Prerequisite intros (`docs/USER_GUIDE_LOCAL.md:6`, `README.md:9`) omit the trusted `~/.local/bin` search added by #74; `USER_GUIDE_LOCAL.md:210` already documents the correct order and is the alignment reference. Docs only. documentation, P3-low. |
| 1.1.1 | #80 update system-lifecycle crash-detection comments to the log-file-scan contract | Coherence (CI-3) | Open | `tests/test-system-lifecycle.bash:777-778` calls `test_crash_detection` disabled/journal-dependent, but it is active at L1478 and passes via the log-file scan (1.1.0 decoupling). Comment-only correction. tests, P3-low. |
...
**Tally:** Done 7 · Open 9 · Not started 0 · In progress 0 · Blocked 0
