...
| M2 | (#128) Layout guard re-breaks stamping on root_squash homes: move the guard's three checks to the same delegated principal as the git queries; the tracked-file check uses repo-top-anchored pathspecs (`:/configure/RULES_INSTALL :/bin/ioc-runner` — a plain relative pathspec resolves under `-C bin` and fails on a genuine checkout); WARN hands over the manual repair when delegation is unavailable; INSTALL.md reconciled | Milestone | Not started (plan reviewed 2026-07-28; ready for code) | Cluster head; fixes the regression this cycle exists for. Three-lens plan review recorded in `work/review-m2-plan.md`: (B1) issue Proposed-Fix item 1 pathspec corrected to `:/`-anchored; (B2) RESOLVED by measurement on both fresh goldens 2026-07-28 — a relative `stat bin` from cwd=repo SUCCEEDS under root_squash (only the absolute-path stat is denied, barrier = the 0750 `gitsrc` ancestor), so the `:634` invoker recovery works even under nested sudo and all three entry points fail only at the guard; moving the guard fixes all three, no scope expansion; (B3) T2 must deploy from `nfs_sim` then assert, or it never goes red. Round-2 review 2026-07-28 (implementation readiness / verification / coherence) + golden measurement: guard-move validated on the golden (delegated checks pass); repair-WARN keys off a `delegation_needed && !delegation_available` flag (direct-root `rev-parse` fails first, so the layout verdict is not the branch); `ls-files` is exit-code-only; the R7-F9 negative is promoted to a permanent suite asset (G5); the doc scope widened to three works-in-place sites and reframed as a restore, not a deletion. Ready for code. |
| M2.T1 | Change-specific: three entry points on the golden `nfs_sim` mount stamp real metadata (after the denial precheck); local-disk unchanged; delegation-unavailable is a separate fixture (`sudo` disabled via `chmod 000 /usr/bin/sudo`, owner resolvable) whose WARN-guided manual repair restores `-V` in the deploy path's UTC format | Verification | Not started | Requires a real `.git` on the mount (the `--exclude=.git` push omits it). Denial precheck (`sudo -n stat` denied to root, allowed to owner) gates trust. Golden-measured 2026-07-28: delegated checks pass (real stamp); direct-root `rev-parse` fails first (empty toplevel), so the repair-WARN keys off the delegation flag, not the layout verdict. |
| M2.T2 | Suites (read-only, per the system-infra contract): (a) reads the deployed `/usr/local/bin/ioc-runner -V` and asserts a bare short hash, rejecting `unknown` AND the `(live)` fallback — a PASS is M2 evidence only alongside the denial-precheck record from the same root_squash deploy; (b) permanent R7-F9 negative — a temp `git init` + `bin/` copy + the stamping step asserts WARN + unknown (no root_squash needed, runs every time; G5 promotion 2026-07-28) | Verification | Not started | |
| M2.T-doc | Docs: M2 RESTORES works-in-place, so keep the claim in all three places (`INSTALL.md:36`, `INSTALL.md:58-69`, `README.md:23`; `UNINSTALL.md:143` cross-ref) and move them in lockstep; re-anchor the 1.2.1-era "verified on alsucl-psrv3" line to the 1.2.2 golden gate; add the delegation-unavailable caveat + a focused TWO-line post-WARN repair (`RUNNER_GIT_HASH`, `RUNNER_COMMIT_DATE`, UTC-normalized) that verbatim-matches the WARN string | Verification | Not started | AC4 doc deliverable. Done-check is doc-code string match, not "a paragraph exists". |
