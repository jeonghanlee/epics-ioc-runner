...
| M8 | 1.2.0 | #52 review procServ child-exit signals for crash-loop detection | Carry-forward | Open | Reframed by the rs20260612_143435 strategy (C1+H, conv C003): now the C1+H confirmation census feeding the M10/M11 coupled pair. Empirical sub-measurements E1-E4 below. **Campaign COMPLETE 2026-06-17: the full-scale confirmatory census (res20260614_210000, plan v5, both goldens) is the authoritative result and SUPERSEDES the E1-E4 single-run pilots (retired). The recorded cases (C1/C3/C6/C7/C8/C9/C12/C15) feed M9-M11; a reproducibility re-run (2026-06-17, both goldens) PASSED — all 8 case verdicts reproduce res20260614_210000: C8 96 s both (exact), C12 stop-stays-inactive both, C3 ~5.1 MB/day UNDER both (recorded 5.1/4.9, run-to-run variance), C1 ~0.82-0.83 s window both, C9 copytruncate-works/create-broken both, C7 `--ignore=^T` fails / `--autorestartcmd=''` disables both, C6 console survives child crash both, C15 assert inactive+in-band-restart both. Cluster ready for M9->M11->M10 implementation.** Exit-signal semantics feed the M10 `Restart=` decision. **Residual / dependency-inversion (2026-06-22 finding): M8 STAYS OPEN even though its consumers M11 (#67) and M10 (#54) are CLOSED. The empirical census is complete and the design question is resolved (M11 implemented #52 option 2 — post-marker stateful scan), but #52's OWN acceptance criteria are not yet recorded as met: M8.T1/T2/T3 (child-kill-positive, restart-negative, #92 re-run) are Open with no verification, and the disposition of #52's named `The process was killed by signal <n>` criterion vs M11's adopted death-banner signal (`@@@ Child process is shutting down`) needs an explicit recorded judgment, plus the system-wide crash-detection mapping (system-lifecycle 76/76) to #52's criteria. Closing M8 is its own small review (criterion disposition + T1/T2/T3 mapping), NOT a one-click close.** |
