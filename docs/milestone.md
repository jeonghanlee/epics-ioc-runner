...
**Next session entry point:** **1.2.0 RELEASED 2026-07-02** (merge `6c50604`, tag `1.2.0`, GitHub release published, milestone closed 19/19, `release-1.1.0` deleted) — the 1.2.0 cycle is complete with no remaining work. The next cycle (1.3.0 or 1.2.1) opens on its own release branch, where this register restarts (release-cycle procedure; backlog #102 and the ansible-provision review item await ordering). Historical journey below: **M11 (#67) and M10 (#54) both closed 2026-06-22**
...
| M20 | 1.2.0 | release gate (no GitHub issue; defined by `testplan_1.2.0.md` "Release Gate") | Release gate | Done | **Gate verification PASSED 2026-06-30** (M20.T1/T2/T3 all green, both goldens). **Release sequence executed 2026-07-02:** changelog `879f113`, bump `2d53d8d`, merge `6c50604` (--no-ff), annotated tag `1.2.0`, GitHub release published (curated notes), milestone closed (19/19), `release-1.1.0` deleted (two-back rule). |
| M20.T1 | 1.2.0 | cycle batch re-run of all M1-M19 change-specific verifications on the final tree | Test sub | Done | PASS 2026-06-30 both goldens on the final tree (c57ba68): all M1-M19 change-specific checks coexist green (error-handling 148/148 executed==counted; local/system lifecycle green). |
| M20.T2 | 1.2.0 | all four suites, both modes, both goldens, clone-and-test + install-and-test | Test sub | Done | PASS 2026-06-30 both goldens, 0 FAIL: rocky8 error 148 / local(source) 75 / system-infra 40 / system-lifecycle 76; debian13 error 148 / local 76 / system-infra 41 / system-lifecycle 76; `setup --full` OK both. Goldens self-baked on alsucl-psrv3. Install-and-test path adds local-lifecycle installed mode: rocky8 75/75, debian13 76/76 (2026-06-30) -- both clone-and-test (source) and install-and-test (installed) local paths pass. Multi-OS EPICS glob gotcha recorded in `testplan_multiuser.md` (52f8643). |
| M20.T3 | 1.2.0 | `testplan_multiuser.md` executed identically (S6/S11 amendments in effect) | Test sub | Done | PASS 2026-06-30, both goldens, accounts provisioned per run (test_users role): L1-L3 + S1-S11 all pass on rocky8 and debian13. XDG local isolation (L1-L3); shared-unit management (S1), setgid `2770` conf edit (S2), concurrency (S3), attach + remove EOF with socket teardown (S4), cross-op log `r--` (S5), #94 observer hint + sudo-deny (S6), disable/manual/re-enable (S7), CRASH_LOG_PATTERNS_EXTRA post-init warning (S8), IOC_CHDIR warning + `..` hard error / force-no-bypass (S9), layered console/inspect root-gate (S10), sudo glob-vs-regex residual (S11). Harness: `sudo -niu` + per-user XDG; prompt/console commands run under pty+timeout+EOF (`script -qec ... </dev/null`) to avoid no-tty hangs. |

**Tally:** milestones Open 0, Done 20 (M1-M20; 1.2.0 released 2026-07-02) · test subs Open 0 (M12.T1-T4 retired with M12; M20.T1-T3 done), Done 45 (through M7.T1/T2, M8.T1/T2/T3, M9.T1/T2/T3, M10.T1/T2/T3, M11.T1/T2/T3, M13.T1/T2, M14.T1/T2, M15.T1/T2, M16.T1/T2, M17.T1, M18.T1/T2, M19.T1/T2/T3, M20.T1/T2/T3) · empirical subs (strategy) 4: campaign COMPLETE (res20260614_210000, plan v5, both goldens) supersedes the E1-E4 pilots — E1->C1 (~0.82 s window), E2/E3->C3 (~5 MB/day, U003/U005), E4->C8 (~96 s trajectory; M11 poll landed 2026-06-22, M10 `KillMode=mixed` recovery frozen ~2.3 s both goldens 2026-06-22); reproducibility re-run PASSED 2026-06-17 (both goldens, 8/8 case verdicts reproduce res20260614_210000) · Blocked 0
