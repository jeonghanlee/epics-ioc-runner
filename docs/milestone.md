...
| Conf parser unification (single parse core, trim + last-wins + tab) + divergence fixtures | C2 (R2-F2/R4-F1), R8-G4 | Behavior-visible; needs its own review. |
| FATAL-subset boundary hygiene (U-2 portable class) + golden rerun | R2-F5 | Pairs with E2E probe. |
| E2E restart-supervision probe on goldens (U-7 approved) + #98 tripwire port to three suites + M19 oneshot via systemd | R8-G1/F5/G3 | Test-infrastructure block. |
| Local-install deploy-after-gates reordering | R1-F3 | do_install flow refactor. |
...
**Tally:** milestones Open 10 (M1-M9 + M10 gate) · carry-forward (1.3.0) 6 recorded · blocked 0.
