...
| M13 | 1.2.1 | (#126) ASCII cleanup of the four 1.2.1-introduced em-dash output strings (two in setup-system-infra, two in ioc-runner) | Release prep | Open | Split from the #121 polish batch to its own 1.2.1 issue. Replacement character decision pending (semicolon recommended). Items 1/2/4/5 of #121 stay in the Backlog. |
| M14 | 1.2.1 | (#125) FAQ Q10 remove failure-mode guidance + CHANGELOG 1.2.1 section; OWNER DECISION pending: include the Operational-notes block for the changed exit-code contracts | Release prep | Open | The two doc halves land together; the CHANGELOG section is a release-sequence prerequisite. |
