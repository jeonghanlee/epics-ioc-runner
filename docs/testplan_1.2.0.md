...
| M10 | #54 | `Restart=` policy from the M8 findings (U008 criterion): under `Restart=always`+`StartLimitIntervalSec=0` a crash-looping IOC stays `activating (auto-restart)`, never systemd `failed`; `KillMode=mixed` bounds procServ-death recovery to seconds (not the ~96 s `control-group` stall). Re-runs the M5 guard (six new rows pinned) and the crash-detection cases. | Both lifecycle suites green; recovery measured seconds-not-~96s and `=0` never-failed on both goldens. |
