...
| Privileged `systemctl` verbs (`start`/`stop`/`restart`/`enable`/`disable`/`daemon-reload`) on `epics-@<name>.service` gated by sudoers to `%ioc`; non-`ioc` `sudo systemctl start epics-@<name>.service` rejected; on sudo >= 1.9.10 hosts an `ioc`-group member's `sudo -n -l /usr/bin/systemctl start 'epics-@bad name.service'` is also rejected by the anchored-regex form | n/a | T-C2d-S | required | required | n/a | required |
...
| B-2 | V-B-2 | `ioc-runner --local install <conf>`; `grep -E '^(UMask\|ExecStart)' ~/.config/systemd/user/epics-@.service` shows `UMask=0027` and `--logfile=${LOG_DIR}/%i.log` where `${LOG_DIR}` is the resolved local log directory (`${LOCAL_LOG_DIR}` by default, overridden by `IOC_RUNNER_LOG_DIR` or `IOC_RUNNER_LOCAL_LOG_DIR`); `stat ${LOG_DIR}` returns `<user>:<user> 750`; repeated install in same second produces two distinct `~/.config/systemd/user/epics-@.service.bak.*` files |
| B-3 | V-B-3 | `logrotate -d /etc/logrotate.d/procserv`; `logrotate -f /etc/logrotate.d/procserv`; `ioc-runner restart <ioc>` |
| C1 | V-C1 | `bash tests/run-all-tests.bash --local` STEP 25 (byte-offset crash detection) on 1.1.0 HEAD; journal-independent detection by a non-`systemd-journal` operator is exercised by T1 (system mode) |
| C2 | V-C2 | Case 1 (procServ-created): `stat -c '%U:%G %a' ${SYSTEM_LOG_DIR}/<ioc>.log` returns `ioc-srv:ioc 644`; `sudo -u <ioc-member> cat <log>` succeeds; `getfacl` shows `mask::r--` (procServ's `0644` mode_arg restricts mask to `r--`). Case 2 (engineer-created, default ACL effect): engineer in `ioc` runs `touch ${SYSTEM_LOG_DIR}/probe.log` under shell `umask 0022`; `stat -c '%U:%G %a' ${SYSTEM_LOG_DIR}/probe.log` returns `<engineer>:ioc 664` (`touch` uses `open(0666)` mode_arg; default ACL `g:ioc:rw` + `m::rw` preserves group write); `sudo -u ioc-srv test -w <probe.log>` exits 0. sudoers gate (narrow): as a non-`ioc` user, `sudo /usr/bin/systemctl start epics-@<name>.service` exits with `not allowed to execute`. On sudo >= 1.9.10 hosts, an `ioc`-group member's `runuser -u <ioc-member> -- sudo -n -l /usr/bin/systemctl start 'epics-@bad name.service'` is also denied (anchored-regex form rejects whitespace and other characters outside the runner IOC-name model); the same probe with a valid name is allowed. Non-`ioc` `ioc-runner` invocation and read-only `ioc-runner status`/`is-active` are not gated by sudoers |
...
| E | V-E | `bash tests/run-all-tests.bash --local` (T3, T4) and `sudo -E bash tests/test-system-lifecycle.bash` (T1, T2, T5) clean PASS on 1.1.0 HEAD; T1 and T5 FAIL on `1.0.8` tag; T2 SKIPs (infra-gated); T3 and T4 are baseline-exempt regression guards (pass on both) |
