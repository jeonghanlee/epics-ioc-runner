...
| Release Verification 2 | 2026-08-17 | Fresh `rocky8` and `debian13` consumers at release candidate `d6f9708` | Pass | The shipped two-host suite driver ran twice consecutively: both runs `GATE SUITES PASS hosts=2`, `688 checks`/`170 steps`, all six suites green, `fail=0`. Evidence: `work/gate-suites-20260818T012140Z-*` and `…T012636Z-*`. State-file metadata: absent on both fresh consumers before the runs; the system logrotate run created the default at `root:root 0600` on each, stable and uncorrupted across the second consecutive run (the M13 fix keeps the local validation off it; the system path creating its own default is expected and out of M13 scope). |
| Release Verification 3 | 2026-08-17 | Fresh `rocky8` and `debian13` consumers at release candidate `d6f9708` | Pass | `gate/drivers/control/run-all.bash` on each host: `VERDICT RUN PASS, 14 scenarios pass=14 fail=0 missing=none` on both. Every declared multi-user scenario (L1-L3, S1-S6, S10, S11) passes on each golden. Evidence: `work/m34-scenarios` (per-scenario VERDICT lines). |
